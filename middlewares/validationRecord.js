...
const validationRecord = [
   body("first_name").notEmpty().withMessage("escribe tu nombre").bail().isLength({min: 2}).withMessage('El nombre debe tener al menos 2 caracteres'),
   body("last_name").notEmpty().withMessage("escribe tu apellido").bail().isLength({min: 2}).withMessage('El apellido debe tener al menos 2 caracteres'),
   body("user_name").notEmpty().withMessage("escribe tu nombre de usuario").bail(),
   body("email").notEmpty().withMessage("escribe tu mail").bail().isEmail().withMessage("debes escribir un formato de email valido"),
